...
<body>
	...
	if ($conn)
	{
		$id = $_POST['uid'];
		$pass = $_POST['pass'];
		$sql = "SELECT * FROM OBRS.staff WHERE id='$id' AND password='$pass'";
		$data = mysqli_query($conn, $sql);
		$result = mysqli_num_rows($data);
		if ($result != 0)
			{
				$sql = "UPDATE OBRS.staff SET active=1 WHERE id='$id'";
				mysqli_query($conn, $sql);
				header('Location:http://localhost/SE-OBRS/Staff/staff.php');
			}
	}
	else
	{
