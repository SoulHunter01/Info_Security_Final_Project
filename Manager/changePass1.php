...
<body>
	...
	if ($conn)
	{
		...
		$sql = "SELECT id FROM OBRS.manager WHERE active=1 AND password='$cp'";
		...
		if ($result != 0) {
			if ($np == $cf){
				...
				$id = $result['id'];
				$sql = "UPDATE OBRS.manager SET password='$np' WHERE id='$id'";
				$res = mysqli_query($conn, $sql);
				if ($res) header('Location:http://localhost/SE-OBRS/Manager/manager.php');
			}
			else{
